...
def admin_contratacion():
    ...
    if sec=='flujo':
        ...
        <div class='firma-page'>
          ...
          <div class='firma-grid firma-grid-boceto'>
            <div class='firma-card camera-pro-card'>
              <h3>Activación de cámara</h3><p class='muted2'>Captura facial en tiempo real. Al detectar rostro se tomará captura automática y sonará confirmación.</p>
              <div class='cam-wrap cam-boceto'><video id='firmaVideo' autoplay playsinline muted></video><canvas id='firmaCanvas' style='display:none'></canvas><img id='firmaPreview' style='display:none'><div class='face-frame'></div><div id='liveBadge' class='live-badge'>● EN ESPERA</div><div id='captureToast' class='capture-toast'>✅ Rostro reconocido correctamente<br><small>Captura realizada automáticamente</small></div></div>
              <div id='soundBox' class='sound-ok' style='display:none'>🔊 ¡Captura exitosa! <span>Rostro reconocido correctamente</span></div>
              <div class='firma-actions'><button type='button' class='c-btn' onclick='firmaStartCam()'>📷 Activar cámara</button><button type='button' class='c-btn green' onclick='firmaCapture()'>📸 Capturar evidencia</button><button type='button' class='c-btn gray' onclick='firmaStopCam()'>■ Detener</button></div><p id='firmaCamMsg' class='muted2'></p>
            </div>
            <div class='firma-card docs-sign-panel'>
              <h3>Documentos a firmar <span id='docsBadge' class='badge-green'>0</span></h3><p class='muted2'>Se firmarán automáticamente los documentos marcados.</p>
              ...
              <label class='switch-row'><input type='checkbox' id='firmaMasivaSwitch' checked onchange='marcarTodosFirma(this.checked)'><span>Firma masiva (todos los documentos)</span></label>
              <div class='ready-box'>✅ Todo listo para firmar<br><small>Los documentos seleccionados se firmarán automáticamente.</small></div>
            </div>
          </div>
          ...
          <form method='post' class='firma-card firma-form' onsubmit='return prepararFirmaMasiva()'>
            ...
            <h3>✅ 3. Firma masiva / lote con check</h3><div class='mass-alert'><b id='firmaMassCounter'>0 seleccionados</b><span>Selecciona contratos y genera enlaces de firma en lote.</span></div>
        ...
        <style>
        .firma-page{{color:#102033!important;background:#eef3f8;margin:-10px -12px 0;padding:20px;min-height:calc(100vh - 90px)}}.firma-page *{{text-shadow:none!important;box-sizing:border-box!important}}.firma-page h1,.firma-page h2,.firma-page h3,.firma-page label,.firma-page p,.firma-page span{{opacity:1!important}}.firma-page h2,.firma-page h3{{color:#07182e!important}}.firma-head h2,.firma-head p{{color:white!important}}.firma-note{{grid-column:1/-1;margin:0 0 4px;color:#475569!important;font-weight:800;line-height:1.45}}.firma-head{{display:flex;justify-content:space-between;gap:16px;align-items:center;background:linear-gradient(135deg,#073552,#0c745f);color:white;border-radius:24px;padding:24px;box-shadow:0 18px 40px #0f172a22}}.firma-head h2{{font-size:34px;margin:0 0 6px}}.firma-head p{{margin:0;color:#dff7ff;font-weight:800}}.firma-head-actions,.firma-actions{{display:flex;gap:10px;flex-wrap:wrap}}.firma-kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:16px 0}}.firma-kpis div{{background:white;border-radius:20px;padding:18px;border:1px solid #d8e4ef;box-shadow:0 12px 28px #0f172a12}}.firma-kpis b{{display:block;font-size:30px;color:#03735f}}.firma-kpis span{{color:#64748b;font-weight:900}}.firma-grid{{display:grid;grid-template-columns:1.35fr .8fr;gap:16px}}.firma-card{{background:white;border:1px solid #d8e4ef;border-radius:22px;padding:20px;margin:16px 0;box-shadow:0 12px 28px #0f172a12;color:#111827}}.firma-card h3{{margin:0 0 12px;font-size:23px;color:#07182e}}.cam-wrap{{background:#07182e;border-radius:20px;min-height:330px;display:grid;place-items:center;overflow:hidden;border:4px solid #dbeafe;position:relative}}.cam-wrap:before{{content:'CÁMARA WEB / FACIAL';position:absolute;inset:auto auto 12px 14px;color:#dbeafe;font-weight:950;font-size:12px;letter-spacing:.5px;opacity:.55;z-index:0}}.cam-wrap video,.cam-wrap img{{width:100%;height:100%;min-height:330px;max-height:430px;object-fit:cover;background:#000;position:relative;z-index:1}}.flow-steps{{display:grid;gap:10px}}.flow-steps span,.trace-grid span{{background:#e0f2fe;border:1px solid #7dd3fc;color:#075985;padding:12px;border-radius:14px;font-weight:950}}.firma-form{{display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:end}}.firma-form h3,.firma-form .wide,.firma-form .mass-alert,.firma-form .table-wrap,.firma-form .firma-actions{{grid-column:1/-1}}.form-row label{{display:block;font-weight:950;margin-bottom:6px;color:#334155}}.form-row input,.form-row select{{background:#f8fafc!important;color:#0f172a!important;border:1px solid #cbd5e1!important;border-radius:14px!important;padding:13px!important;width:100%!important;min-height:54px!important;font-weight:900!important}}.firma-page .c-btn{{border:0!important;border-radius:14px!important;padding:13px 18px!important;font-weight:1000!important;cursor:pointer!important;text-decoration:none!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;background:linear-gradient(135deg,#ffb21a,#ffd23f)!important;color:#07111d!important;box-shadow:0 10px 22px #0f172a18!important}}.firma-page .c-btn.gray{{background:#334155!important;color:white!important}}.firma-page .c-btn:disabled{{opacity:1!important;filter:none!important;cursor:pointer!important}}.mass-alert{{display:flex;justify-content:space-between;gap:10px;align-items:center;background:#dcfce7;border:2px solid #22c55e;color:#166534;border-radius:18px;padding:14px 16px;font-weight:950}}.firma-table th{{background:#073552!important;color:white!important}}.firma-table td{{color:#1f2937!important;background:white!important}}.estado-pill,.estado-soft,.ok-chip,.pend-chip{{display:inline-flex;border-radius:999px;padding:7px 10px;font-weight:950}}.estado-pill{{background:#fef3c7;color:#92400e}}.estado-soft{{background:#e0f2fe;color:#075985}}.ok-chip{{background:#dcfce7;color:#166534}}.pend-chip{{background:#fee2e2;color:#991b1b}}.trace-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}}.c-btn.green{{background:linear-gradient(135deg,#16a34a,#22c55e)!important;color:white!important}}.firma-grid-boceto{{grid-template-columns:1.05fr .95fr!important}}.camera-pro-card,.docs-sign-panel{{min-height:620px}}.cam-boceto{{min-height:430px;border:4px solid #bfdbfe!important;background:#000!important}}.cam-boceto video,.cam-boceto img{{min-height:430px!important}}.face-frame{{position:absolute;z-index:3;inset:25% 23%;border:3px solid #22c55e;border-radius:20px;box-shadow:0 0 0 999px rgba(0,0,0,.04);pointer-events:none;display:none}}.cam-live .face-frame{{display:block}}.live-badge{{position:absolute;right:18px;top:18px;z-index:4;background:rgba(15,23,42,.78);color:#fff;border-radius:999px;padding:8px 12px;font-weight:1000;font-size:12px}}.capture-toast{{position:absolute;left:22px;bottom:22px;z-index:5;background:rgba(15,23,42,.88);color:white;border-radius:16px;padding:14px 18px;font-weight:1000;display:none}}.capture-toast small{{color:#d1fae5}}.capture-ok .capture-toast{{display:block}}.sound-ok{{margin:12px 0;background:#dcfce7;border:1px solid #bbf7d0;color:#166534;border-radius:14px;padding:13px 16px;font-weight:1000}}.sound-ok span{{margin-left:10px;color:#15803d}}.badge-green{{display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;border-radius:999px;background:#16a34a;color:white;font-size:14px}}.doc-sign-list{{display:grid;gap:12px;margin:18px 0;max-height:430px;overflow:auto;padding-right:4px}}.doc-sign-card{{display:grid;grid-template-columns:34px 54px 1fr;gap:12px;align-items:center;border:1px solid #dbe3ec;border-radius:14px;background:#f8fafc;padding:14px;cursor:pointer;box-shadow:0 8px 18px #0f172a0d}}.doc-sign-card input{{width:22px;height:22px;accent-color:#16a34a}}.doc-icon{{width:46px;height:46px;border-radius:12px;background:#2563eb;color:white;display:grid;place-items:center;font-weight:1000;font-size:20px}}.doc-info b{{display:block;color:#0f172a;font-size:14px}}.doc-info small{{display:block;color:#64748b;font-weight:800;margin-top:3px}}.switch-row{{display:flex;gap:12px;align-items:center;margin:14px 0;font-weight:1000;color:#0f172a}}.switch-row input{{width:22px;height:22px;accent-color:#16a34a}}.ready-box{{background:#dcfce7;border:1px solid #bbf7d0;border-radius:14px;padding:14px;color:#166534;font-weight:1000}}.ready-box small{{color:#15803d}}.empty-docs{{background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;border-radius:14px;padding:14px;font-weight:900}}@media(max-width:1100px){{.firma-grid-boceto{{grid-template-columns:1fr!important}}.camera-pro-card,.docs-sign-panel{{min-height:auto}}}}@media(max-width:900px){{.firma-head,.firma-grid{{grid-template-columns:1fr;display:grid}}.firma-kpis{{grid-template-columns:repeat(2,1fr)}}.firma-form{{grid-template-columns:1fr}}.trace-grid{{grid-template-columns:1fr}}}}
        </style>
        <script>
        ...
        async function firmaStartCam(){{const msg=document.getElementById('firmaCamMsg'),wrap=document.querySelector('.cam-wrap'),badge=document.getElementById('liveBadge');try{{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{msg.textContent='Tu navegador no permite cámara en este contexto. Usa Chrome/Edge actualizado con HTTPS o localhost.';return;}}if(location.protocol!=='https:' && location.hostname!=='localhost' && location.hostname!=='127.0.0.1'){{msg.textContent='Para celular/navegador real, publica en HTTPS o prueba en localhost. Intentando activar cámara...';}}let constraints={{video:{{facingMode:'user',width:{{ideal:1280}},height:{{ideal:720}}}},audio:false}};try{{firmaStream=await navigator.mediaDevices.getUserMedia(constraints);}}catch(e1){{firmaStream=await navigator.mediaDevices.getUserMedia({{video:true,audio:false}});}}const v=document.getElementById('firmaVideo');v.srcObject=firmaStream;v.muted=true;await v.play();if(wrap)wrap.classList.add('cam-live');if(badge)badge.textContent='● EN VIVO';msg.textContent='Cámara activa. Buscando rostro y preparando captura automática...';setTimeout(async()=>{{try{{if('FaceDetector' in window){{const fd=new FaceDetector({{fastMode:true,maxDetectedFaces:1}});const faces=await fd.detect(v);if(faces&&faces.length){{msg.textContent='Rostro detectado. Capturando evidencia y validando...';firmaCapture();return;}}msg.textContent='Captura automática de respaldo por compatibilidad del navegador.';}}}}catch(_e){{}}firmaCapture();}},1600);}}catch(e){{msg.textContent='No se pudo activar cámara: '+(e&&e.name?e.name:'permiso denegado')+'. Revisa permisos del navegador, usa HTTPS/localhost y cierra Zoom/Teams u otras apps que usen la cámara.';}}}}
        function firmaCapture(){{const v=document.getElementById('firmaVideo'),c=document.getElementById('firmaCanvas'),img=document.getElementById('firmaPreview'),msg=document.getElementById('firmaCamMsg'),wrap=document.querySelector('.cam-wrap'),sound=document.getElementById('soundBox');if(!v.videoWidth){{msg.textContent='Primero activa la cámara.';return;}}c.width=v.videoWidth;c.height=v.videoHeight;c.getContext('2d').drawImage(v,0,0);img.src=c.toDataURL('image/png');img.style.display='block';if(wrap)wrap.classList.add('capture-ok');if(sound)sound.style.display='block';firmaBeep();msg.textContent='Evidencia facial capturada automáticamente. Lista para firmar todos los documentos seleccionados.';}}
        function firmaStopCam(){{if(firmaStream){{firmaStream.getTracks().forEach(t=>t.stop());firmaStream=null;}}const b=document.getElementById('liveBadge');if(b)b.textContent='● DETENIDA';document.getElementById('firmaCamMsg').textContent='Cámara detenida.';}}
        function updateFirmaCounter(){{const n=[...document.querySelectorAll('.chk-doc-firma:checked')].length;const el=document.getElementById('firmaMassCounter');if(el)el.textContent=n+' seleccionados';const b=document.getElementById('docsBadge');if(b)b.textContent=n;}}
        function marcarTodosFirma(on){{document.querySelectorAll('.chk-doc-firma').forEach(x=>x.checked=on);updateFirmaCounter();}}
        function prepararFirmaMasiva(){{const ids=[...new Set([...document.querySelectorAll('.chk-doc-firma:checked')].map(x=>x.value))];if(ids.length===0){{alert('Selecciona al menos un contrato para envío masivo.');return false;}}document.getElementById('documentos_lote').value=ids.join('\n');return confirm('Se generarán '+ids.length+' enlace(s) de firma. ¿Continuar?');}}
        document.addEventListener('change',e=>{{if(e.target.classList&&e.target.classList.contains('chk-doc-firma'))updateFirmaCounter();}});
        document.addEventListener('DOMContentLoaded',updateFirmaCounter);
        </script>
        """)
    elif sec=='nisira':
