...
def login_template(admin=False, error=""):
    ...
    if admin:
        ...
        fields = f"""
          <div class='field'><label>Empresa</label><div class='login-input'>🏢<select name='empresa' required><option value=''>Seleccione empresa</option>{opts}</select></div></div>
          <div class='field'><label>Usuario</label><div class='login-input'>👤<input name='usuario' placeholder='admin' required></div></div>
          <div class='field'><label>Clave</label><div class='login-input'>🔒<input name='clave' type='password' placeholder='••••••••' required></div></div>
        """
    else:
        opts = ''.join([f"<option value='{e}'>{e}</option>" for e in empresas_disponibles_login()])
        fields = f"""
