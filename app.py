...
def admin_contratacion():
    ...
    if sec=='flujo':
        ...
        async function firmaStartCam(){{const msg=document.getElementById('firmaCamMsg'),wrap=document.querySelector('.cam-wrap'),badge=document.getElementById('liveBadge');try{{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{msg.textContent='Tu navegador no permite cámara. Usa Chrome/Edge con HTTPS o localhost.';return;}}let constraints={{video:{{facingMode:'user',width:{{ideal:1280}},height:{{ideal:720}}}},audio:false}};try{{firmaStream=await navigator.mediaDevices.getUserMedia(constraints);}}catch(e1){{firmaStream=await navigator.mediaDevices.getUserMedia({{video:true,audio:false}});}}const v=document.getElementById('firmaVideo');v.srcObject=firmaStream;v.muted=true;await v.play();if(wrap){{wrap.classList.add('cam-live');wrap.classList.remove('capture-ok');}}if(badge)badge.textContent='● EN VIVO';msg.textContent='Cámara activa. Buscando rostro y preparando captura automática...';setTimeout(()=>firmaCapture(),1400);}}catch(e){{msg.textContent='No se pudo activar cámara: '+(e&&e.name?e.name:'permiso denegado')+'. Revisa permisos del navegador.';}}}}
