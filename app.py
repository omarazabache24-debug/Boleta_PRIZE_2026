...
def admin_contratacion():
    ...
    if sec=='flujo':
        ...
        async function firmaStartCam(){{const msg=document.getElementById('firmaCamMsg');try{{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{msg.textContent='Tu navegador no permite cámara en este contexto. Usa Chrome/Edge actualizado con HTTPS o localhost.';return;}}if(location.protocol!=='https:' && location.hostname!=='localhost' && location.hostname!=='127.0.0.1'){{msg.textContent='Para celular/navegador real, publica en HTTPS o prueba en localhost. Intentando activar cámara...';}}let constraints={{video:{{facingMode:'user',width:{{ideal:1280}},height:{{ideal:720}}}},audio:false}};try{{firmaStream=await navigator.mediaDevices.getUserMedia(constraints);}}catch(e1){{firmaStream=await navigator.mediaDevices.getUserMedia({{video:true,audio:false}});}}const v=document.getElementById('firmaVideo');v.srcObject=firmaStream;v.muted=true;await v.play();msg.textContent='Cámara activa. Buscando rostro y preparando captura automática...';setTimeout(async()=>{{try{{if('FaceDetector' in window){{const fd=new FaceDetector({{fastMode:true,maxDetectedFaces:1}});const faces=await fd.detect(v);if(faces&&faces.length){{msg.textContent='Rostro detectado. Capturando evidencia y validando...';firmaCapture();return;}}msg.textContent='No se detectó rostro claro; se hará captura de respaldo.';}}}}catch(_e){{}}firmaCapture();}},1800);}}catch(e){{msg.textContent='No se pudo activar cámara: '+(e&&e.name?e.name:'permiso denegado')+'. Revisa permisos del navegador, usa HTTPS/localhost y cierra Zoom/Teams u otras apps que usen la cámara.';}}}}
